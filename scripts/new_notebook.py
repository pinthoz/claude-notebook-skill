...
def load_template(skill_dir: Path) -> dict[str, Any]:
    asset_name = "experiment-template.ipynb"
...
def update_title(notebook: dict[str, Any], title: str) -> None:
    expected = f"# Experiment: {title}\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scaffold an academic experiment notebook.")
    parser.add_argument(
        "--title",
        required=True,
...
def main() -> None:
    ...
    notebook = load_template(skill_dir)
    update_title(notebook, args.title)
    ...
    print(f"Wrote {out_path} using academic experiment template.")
